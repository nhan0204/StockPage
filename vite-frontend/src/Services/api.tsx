...
const finPrepApiKey = import.meta.env.VITE_FIN_PREP_API_KEY;
const finHubApiKey = import.meta.env.VITE_FIN_HUB_API_KEY;
...
export const getIncomeStatement = async(query: string) => {
    ...
    const respone = await fetchData<GetIncomeStatementResponse>(request);
    return respone;
}
...
interface GetDailyDividendResponse {
    data: CompanyDividend[];
}

export const getDailyDividend = async(query: string) => {
    const request = `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${query}&outputsize=full&apikey=${alphaApiKey}`;
    const response = await fetchData<GetDailyDividendResponse>(request);
    return response;
}